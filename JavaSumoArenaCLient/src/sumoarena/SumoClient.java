...
public class SumoClient {
	...
	public static void main(String[] args) {
		...
		while (true) {
			try {
				gameClient = new GameClient(hostname, port, name, avatar, verbose);
	...
	private static void readOptions(String[] args) {
		...
		if (!options.check(true, false)) {
			System.out
					.println("usage: java SumoClient -name=<your client name> [-hostname=<server>] [-port=<server port number>] [-avatar=<url of the image>] [-verbose]");
			System.exit(1);
		}
